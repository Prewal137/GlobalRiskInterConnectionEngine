"""
🤖 Economy Sector Model Training (Phase 5)

This script trains an XGBoost model to predict future economic risk.

Input: 
    - data/processed/economy/economy_features.csv
    - models/trained/economy_scaler.pkl

Output:
    - models/trained/economy_model.pkl
    - data/processed/economy/economic_risk_output.csv

Key Features:
    - Strict time-based train-test split (no leakage)
    - Proper scaler application (fit on train only)
...
MODEL_OUTPUT = os.path.join(BASE_PATH, "models", "trained", "economy_model.pkl")
PREDICTIONS_OUTPUT = os.path.join(BASE_PATH, "data", "processed", "economy", "economic_risk_output.csv")
...
def load_data():
    """
    Load and prepare economy features dataset.
...
def select_features(df):
    ...
    Drop columns:
        - Country, Year, Month (metadata)
        - economic_risk (current target - not used for prediction)
    
    Target: economic_risk_next (future risk)
    ...
    # Define features (all except dropped columns)
    feature_cols = [col for col in df.columns if col not in drop_cols]
    
    X = df[feature_cols].copy()
    y = df['economic_risk_next'].copy()
...
def apply_scaler(X_train, X_test, scaler_path=SCALER_PATH):
    """
    Load pre-fitted scaler and transform features.
    
    CRITICAL: Do NOT fit scaler again - use saved scaler from feature engineering
    ...
    # Identify base feature columns (the ones that were scaled during feature engineering)
    ...
    # Apply scaler ONLY to base features
    X_train_scaled[base_features] = scaler.transform(X_train[base_features])
    X_test_scaled[base_features] = scaler.transform(X_test[base_features])
    
    print(f"   ✅ Scaler applied to base features: {base_features}")
    print(f"   ⚠️  Other features left unscaled (already in appropriate ranges)")
...
def create_model():
    ...
    """
    print("\n" + "="*70)
    print("🤖 STEP 5: MODEL CREATION (XGBOOST)")
    print("="*70)
...
def save_predictions(test_df, y_pred, output_path=PREDICTIONS_OUTPUT):
    ...
    output_df = pd.DataFrame({
        ...
        'Year': test_df['Year'],
        'Month': test_df['Month'],
        'actual_risk': test_df['economic_risk_next'].values,
...
def plot_actual_vs_predicted(test_df, y_pred):
    ...
    """
    print("\n" + "="*70)
    print("📈 STEP 12: VISUALIZATION (ACTUAL VS PREDICTED)")
    print("="*70)
    ...
    try:
        ...
        ax1.plot(range(len(test_df_copy)), test_df_copy['economic_risk_next'], 
        ...
        ax1.set_ylabel('Economic Risk Score', fontsize=12)
        ax1.set_title('Economic Risk: Actual vs Predicted (Test Set)', fontsize=14, fontweight='bold')
        ax1.legend(loc='best', fontsize=10)
        ax1.grid(True, alpha=0.3)
        
        # Plot 2: Scatter plot
        ax2 = axes[1]
        ax2.scatter(test_df_copy['economic_risk_next'], y_pred, alpha=0.6, s=50)
        
        # Add diagonal line
        min_val = min(test_df_copy['economic_risk_next'].min(), y_pred.min())
        max_val = max(test_df_copy['economic_risk_next'].max(), y_pred.max())
        ax2.plot([min_val, max_val], [min_val, max_val], 'r--', linewidth=2, label='Perfect Prediction')
        
        ax2.set_xlabel('Actual Economic Risk', fontsize=12)
        ax2.set_ylabel('Predicted Economic Risk', fontsize=12)
        ax2.set_title('Actual vs Predicted Economic Risk (Test Set)', fontsize=14, fontweight='bold')
        ...
        # Save plot
        plot_path = os.path.join(BASE_PATH, "models", "evaluation", "economy_actual_vs_predicted.png")
...
def main():
    ...
    print("\n" + "="*70)
    print("🤖 ECONOMY SECTOR MODEL TRAINING (PHASE 5)")
    print("="*70)
    ...
    try:
        ...
        # STEP 3: SELECT FEATURES
        # ============================================================
        X_train_full, y_train_full, feature_names = select_features(train_df)
        X_test_full, y_test_full, _ = select_features(test_df)
        
        # ============================================================
        # STEP 4: APPLY SCALER
        # ============================================================
        X_train, X_test = apply_scaler(X_train_full, X_test_full)
        ...
        # STEP 6: TRAIN MODEL
        # ============================================================
        model = train_model(model, X_train, y_train_full)
        ...
        # STEP 8: EVALUATION
        # ============================================================
        metrics = evaluate_model(y_test_full, y_pred)
        ...
        # ============================================================
        print("\n" + "="*70)
        print("✅ ECONOMY MODEL TRAINING COMPLETED SUCCESSFULLY")
        print("="*70)
        print(f"\n   📊 FINAL RESULTS:")
        ...
        print(f"      Plot: models/evaluation/economy_actual_vs_predicted.png")
