...
and geopolitics risks with calculated impact metrics.
...
def get_risk_by_date_cached(country: str, year: int, month: int):
    ...
    df = reconstruct_dataframe(records, [
        ...
        'trade_risk', 'geopolitical_risk', 'economic_impact', 'trade_impact', 'global_risk'
    ])
...
def get_trend_data_cached(country: str) -> tuple:
    ...
    for _, row in df_last_12.iterrows():
        trend_data.append({
            ...
            "economic_impact": round(float(row['economic_impact']), 4),
            "trade_impact": round(float(row['trade_impact']), 4)
        })
